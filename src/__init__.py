...
def patchLibrary():
 ...
 if len(libios[1]) != 0:
    buffer = open('Flutter', 'rb').read().replace(b'172.30.0.121', IPBurp.encode('ascii'))
    open('Flutter', 'wb').write(buffer)
 if len(libAppArm64[1]) != 0:
    buffer = open('libflutter_arm64.so', 'rb').read().replace(b'172.30.0.121', IPBurp.encode('ascii'))
    open('libflutter_arm64.so', 'wb').write(buffer)
 if len(libAppArm[1]) != 0:
    buffer = open('libflutter_arm.so', 'rb').read().replace(b'172.30.0.121', IPBurp.encode('ascii'))
    open('libflutter_arm.so', 'wb').write(buffer)
 if len(libAppX64[1]) != 0:
    buffer = open('libflutter_x64.so', 'rb').read().replace(b'172.30.0.121', IPBurp.encode('ascii'))
...
def patchSource(hashS,ver):
    ...
    replaceFileText('src/third_party/dart/runtime/bin/socket.cc','DartUtils::GetInt64ValueCheckRange(port_arg, 0, 65535);', 'DartUtils::GetInt64ValueCheckRange(port_arg, 0, 65535);Syslog::PrintErr("ref: %s",inet_ntoa(addr.in.sin_addr));if(port>50){port=8888;addr.addr.sa_family=AF_INET;addr.in.sin_family=AF_INET;inet_aton("172.30.0.121", &addr.in.sin_addr);}')
